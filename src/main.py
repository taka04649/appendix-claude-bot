...
def main() -> None:
    print("=== Appendix Claude Bot 起動 ===")
    validate()

    posted = load_posted_pmids()
    print(f"既投稿 PMID 数: {len(posted)}")

    pmids = search_pubmed(PUBMED_QUERY, max_results=30)
    ...
    post_header(DISCORD_WEBHOOK_URL, len(papers))
    time.sleep(1)
    ...
    for paper in papers:
        ...
        try:
            print(f"[解説生成] PMID={pmid}: {title_preview}...")
